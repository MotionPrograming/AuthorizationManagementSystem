...
public class AuthController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if ("/2fa-login".equals(path)) {
				twoFactorLogin(req, resp);
				return;
			}
			...
			if (msg.startsWith("2FA_REQUIRED:")) {
				...
				resp.getWriter().write(JsonUtil.response("ERROR", msg));
			}
	...
	private void login(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (isJsonRequest(req)) {
			...
			try {
				...
				request = JsonUtil.fromJson(readRequestBody(req), LoginRequest.class);
	...
	private void register(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (isJsonRequest(req)) {
			...
			try {
				...
				request = JsonUtil.fromJson(readRequestBody(req), CreateUserRequest.class);
	...
	private void changePassword(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (user == null) {
			...
			resp.getWriter().write(JsonUtil.response("ERROR", "Unauthorized"));
	...
	private Long parseLong(String value) {
		...
		try {
			...
			throw new com.ams.common.exception.ValidationException("userId is required");
		}
	}

	private Integer parseCode(String value) {
		...
		try {
			...
			throw new com.ams.common.exception.ValidationException("A valid 6-digit code is required");
		}
	}
}